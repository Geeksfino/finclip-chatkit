...
final class MainChatViewController: UIViewController {
  ...
  private lazy var modeToggleButton: UIButton = {
    ...
    button.translatesAutoresizingMaskIntoConstraints = false
    button.addTarget(self, action: #selector(modeToggleTapped), for: .touchUpInside)
    return button
  }()
  ...
  private func setupUI() {
    ...
    topBarView.addSubview(modeToggleButton)
    ...
    NSLayoutConstraint.activate([
      ...
      modeToggleButton.trailingAnchor.constraint(equalTo: addButton.leadingAnchor, constant: -12),
      modeToggleButton.centerYAnchor.constraint(equalTo: topBarView.centerYAnchor),
  ...
  func createNewConversation() {
    Task { @MainActor in
      do {
        ...
        showAlert(title: "Error", message: "Failed to create conversation. Please try again.")
      }
    }
  }
  ...
  @objc private func modeToggleTapped() {
    let alert = UIAlertController(title: "Connection Mode", message: "Choose how to connect to the AI agent", preferredStyle: .actionSheet)
    
    alert.addAction(UIAlertAction(title: "Remote Server", style: .default) { [weak self] _ in
      self?.switchMode(.remote)
    })
    
    alert.addAction(UIAlertAction(title: "Local LLM (Gemma 270M)", style: .default) { [weak self] _ in
      self?.switchMode(.local)
    })
    ...
    if let popover = alert.popoverPresentationController {
      popover.sourceView = modeToggleButton
      popover.sourceRect = modeToggleButton.bounds
    }
    
    present(alert, animated: true)
  }
  
  private func switchMode(_ mode: ConnectionMode) {
    AppConfig.currentMode = mode
    
    // Get SceneDelegate to switch mode
    guard let sceneDelegate = view.window?.windowScene?.delegate as? SceneDelegate else {
      print("⚠️ [MainChatViewController] Could not access SceneDelegate")
      return
    }
    
    sceneDelegate.switchMode(mode)
    updateModeToggleButton()
    
    let modeName = mode == .remote ? "Remote Server" : "Local LLM"
    showAlert(title: "Mode Switched", message: "Now using \(modeName)")
  }
  ...
  private func updateModeToggleButton() {
    ...
    case .remote:
      image = UIImage(systemName: "cloud", withConfiguration: imageConfig)
      title = "Remote"
    case .local:
      image = UIImage(systemName: "cpu", withConfiguration: imageConfig)
      title = "Local"
    }
    
    var buttonConfig = modeToggleButton.configuration ?? UIButton.Configuration.plain()
    buttonConfig.image = image
    var attributedTitle = AttributedString(title)
    attributedTitle.font = .systemFont(ofSize: 12, weight: .medium)
    buttonConfig.attributedTitle = attributedTitle
    modeToggleButton.configuration = buttonConfig
  }
}
